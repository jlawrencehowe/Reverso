...
public class PlayerController : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (!gameOver) {
            ...
            if (Input.GetAxisRaw ("Dash") != 0 && dashTimer <= 0 && !isDashing && dashCD <= 0) {
				isDashing = true;
				dashDir = moveDir;
				dashTimer = 0.1f;
			}
			...
			//Right Trigger Input
			if (Input.GetAxisRaw ("Fire1") != 0 && !isDashing) {
		...
		/*	//throw ball input
			...
			if(this.GetComponent<HealthController>().isInvulerable){
	...
	public void OnTriggerEnter (Collider coll)
	{
		...
		if (coll.gameObject.tag == "TripleShot") {
            ...
            ShoutCaster.PlayPowerUp("Triple");
            ...
            ShoutCaster.PlayPowerUp("Health");
            ...
            ShoutCaster.PlayPowerUp("Power");
        } else if (coll.gameObject.tag == "Invulnerability") {
			
			this.GetComponent<HealthController> ().isInvulerable = true;
			this.GetComponent<HealthController> ().invulnerableTimer = 8;
			Destroy (coll.gameObject);
			powerUps.PlayOneShot(invulnerability);

            ShoutCaster.PlayPowerUp("Invincibility");
        } 
